...
```bash
lp-is-gamestore$ sam local invoke helloFromLambdaFunction --no-event
```
...
```bash
lp-is-gamestore$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
lp-is-gamestore$ dotnet test test/lp-is-gamestore.Tests/lp-is-gamestore.Tests.csproj 
```
...
```bash
sam delete --stack-name lp-is-gamestore
```
